...
class Band_suit(nn.Module):
    ...
    def forward(self, x):
        ...
        x_fused = x + x2*torch.sigmoid(x1)
        return x_fused
class Embeddings(nn.Module):
    """Construct the embeddings from patch, position embeddings.
    """
    def __init__(self, config, img_size, backbone, image_band =11,in_channels=3):
        ...
        self.band_suit = Band_suit(width=self.width)
...
class DecoderCup(nn.Module):
    ...
    def forward(self, hidden_states, features=None):
        ...
        x2 = self.conv4(x) 
        x2 = self.up2(x2)
        # x2 = torch.cat([x2, features[5],features[6]], dim=1)
        x3 = torch.cat([x1, features[1]], dim=1)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x1,x2,x3 = self.decoder(x1, features) #x1,
        logits1 = self.segmentation_head1(x1)
        logits2 = self.segmentation_head2(x2)
        logits3 = self.segmentation_head1(x3)
